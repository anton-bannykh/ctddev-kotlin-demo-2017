...
fun bfs(g: Array<ArrayList<Edge>>, d: Array<Int>, s: Int, t: Int, a: Int): Boolean {
    val n = g.size
    d.fill(n + 10)
    d[s] = 0
    val que = ArrayQueue<Int>(n)
    que.add(s)
    while (!que.isEmpty()) {
        val v = que.removeAt(0)
        for (e in g[v]) {
            val u = e.u
...
fun dfs(v: Int, minc: Int, g: Array<ArrayList<Edge>>, h: Array<Int>, d: Array<Int>, t: Int): Int {
    if (v == t || minc == 0) {
        return minc
    }
    while (h[v] < g[v].size) {
...
fun dinitz(g: Array<ArrayList<Edge>>, s: Int, t: Int): Array<Int> {
    val d = Array(g.size, { 0 })
    val h = Array(g.size, { 0 })
    var cnt = 0
    var maxc = -1
    for (i in 0..(g.size - 1)) {
        for (e in g[i]) {
...
fun maxFlow(n: Int, m: Int, v: Array<Int>, u: Array<Int>, c: Array<Int>, s: Int, t: Int): Array<Int> {
