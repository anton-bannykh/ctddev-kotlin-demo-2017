fun get(edgeDsu: IntArray, v: Int): Int = if (v == -1) -1 else if (edgeDsu[v] == v) v else {
    edgeDsu[v] = get(edgeDsu, edgeDsu[v]); edgeDsu[v]
}
...
fun runTest(n: Int, queries: Array<Array<Int>>): MutableList<Int> {
    ...
    for (query in queries) {
        when (query[0]) {
            -1 -> {
                bridges = addEdge(query[1], query[2], n + 1, size, parent, compDSU, edgeDSU, bridges)
            }
            -2 -> {
                ans.add(bridges)
            }
